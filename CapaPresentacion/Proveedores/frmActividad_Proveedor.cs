...
{
    public partial class frmActividad_Proveedor : Form
    {
        string Operacion = null;  // Operaciones : N = Nuevo / M = Modificar E = Eliminar
        public frmActividad_Proveedor()
        {
            InitializeComponent();
        }

        private void frmActividad_Proveedor_Load(object sender, EventArgs e)
        {
            FormatoDgv();
            Mostrar_dgv("");
            llenar_CboEstado();
        ...
        void FormatoDgv()
        {
            ...
            dgv.Columns[0].DataPropertyName = "ACTI_PROV_IDE";

            dgv.Columns[1].Width = 240;
            dgv.Columns[1].HeaderText = "Descripción ";
            dgv.Columns[1].DataPropertyName = "ACTI_PROV_NOMBRE";

            dgv.Columns[2].Width = 100;
            dgv.Columns[2].HeaderText = "Detraccion";
            dgv.Columns[2].DataPropertyName = "ACTI_PROV_MONTO_DETRACCION";
            ...
            dgv.Columns[3].DataPropertyName = "ACTI_PROV_ESTADO";


            // Columnas que no Deben Verse

            dgv.Columns[4].DataPropertyName = "ACTI_PROV_FECHAINAC";
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsActividad_ProveedorBC.Listar(filtro);
            if (R.Proceder)
            {
        ...
        private void Mostrar_Datos()
        {
            if (dgvListado.CurrentRow != null)
            {
                ...
                txtDetraccion.Text = Convert.ToDecimal(this.dgvListado.CurrentRow.Cells["DETRACCION"].Value).ToString("0.00");
        ...
        {
            ClsActividad_ProveedorBE TipoBE = new ClsActividad_ProveedorBE();
            TipoBE.Acti_prov_ide = Convert.ToInt32(txtIde.Text);
            TipoBE.Acti_prov_nombre = txtNombre.Text;
            TipoBE.Acti_prov_monto_detraccion = Convert.ToDecimal(txtDetraccion.Text);
            TipoBE.Acti_prov_estado = cboEstado.Text;
            TipoBE.Acti_prov_fechainac = Convert.ToDateTime("01-01-1900");
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsActividad_ProveedorBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Actividad Proveedor..." + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsActividad_ProveedorBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Actividad Proveedor..." + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsActividad_ProveedorBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Actividad Proveedor..." + R.Sms);
                        break;
                    }
            }
