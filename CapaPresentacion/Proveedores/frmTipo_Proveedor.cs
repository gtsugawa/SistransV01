...
{
    public partial class frmTipo_Proveedor : Form
    {
        string Operacion = null;  // Operaciones : N = Nuevo / M = Modificar E = Eliminar
        public frmTipo_Proveedor()
        {
            InitializeComponent();
        }

        private void frmTipo_Proveedor_Load(object sender, EventArgs e)
        {
            FormatoDgv();
            Mostrar_dgv("");
            llenar_CboEstado();
        ...
        void FormatoDgv()
        {
            ...
            dgv.Columns[0].DataPropertyName = "TIPO_PROV_IDE";

            dgv.Columns[1].Width = 340;
            dgv.Columns[1].HeaderText = "Descripción ";
            dgv.Columns[1].DataPropertyName = "TIPO_PROV_NOMBRE";
            ...
            dgv.Columns[2].DataPropertyName = "TIPO_PROV_ESTADO";

            // Columnas que no Deben Verse

            dgv.Columns[3].DataPropertyName = "TIPO_PROV_FECHAINAC";
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsTipo_ProveedorBC.Listar(filtro);
            if (R.Proceder)
            {
        ...
        {
            ClsTipo_ProveedorBE TipoBE = new ClsTipo_ProveedorBE();
            TipoBE.Tipo_prov_ide = Convert.ToInt32(txtIde.Text);
            TipoBE.Tipo_prov_nombre = txtNombre.Text;
            TipoBE.Tipo_prov_estado = cboEstado.Text;
            TipoBE.Tipo_prov_fechainac = Convert.ToDateTime("01-01-1900");
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsTipo_ProveedorBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Tipo Proveedor..." + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsTipo_ProveedorBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Tipo Proveedor..." + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsTipo_ProveedorBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Tipo Proveedor..." + R.Sms);
                        break;
                    }
            }
