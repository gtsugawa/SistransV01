...
{
    public partial class Mantenimiemto_Preventivo : Form
    {
        ...
        public Mantenimiemto_Preventivo()
        {
            InitializeComponent();
        }

        private void Mantenimiemto_Preventivo_Load(object sender, EventArgs e)
        {
            FormatoDgv();
            Inicializa_Fechas();
            Mostrar("");
        ...
        {
            ENResultOperation R = ClsMantenimiento_PreventivoBC.Listar_por_Fechas(dtpFecIni.Value,dtpFecFin.Value);
            if (R.Proceder)
            {
        ...
        {
            ENResultOperation R =  ClsMantenimiento_PreventivoBC.Buscar_Registro(nMant_Ide);
        ...
        private Boolean Verifica_Campos()
        {
            if (iTran_Vehi_Ide==0)
            {
                MessageBox.Show("No se Ha Obtenido Registro del Vehiculo", "Mantenimiento Preventivo");
                return false;
            }
            if (string.IsNullOrEmpty(txtPlaca.Text))
            {
                MessageBox.Show("No se Ha seleccionado Placa del Vehiculo", "Mantenimiento Preventivo");
                return false;
            }
            if (string.IsNullOrEmpty(txtKilometraje.Text))
            {
                MessageBox.Show("No se Ha Ingresado Kilometraje del Vehiculo", "Mantenimiento Preventivo");
                return false;
            }
            if (string.IsNullOrEmpty(txtSolicitado.Text))
            {
                MessageBox.Show("No se Ha seleccionado Campo Solicitado Por", "Mantenimiento Preventivo");
                return false;
            }
        ...
        {
            Reportes.rptMant_Preventivo frmrpt = new Reportes.rptMant_Preventivo();
            ...
            frmrpt.Titulo = "MANTENIMIENTO PREVENTIVO";
            frmrpt.ShowDialog();
        }
        ...
        {
            ClsMantenimiento_PreventivoBE TipoBE = new ClsMantenimiento_PreventivoBE();
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsMantenimiento_PreventivoBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar " + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsMantenimiento_PreventivoBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar " + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsMantenimiento_PreventivoBC.Eliminar(TipoBE);
        ...
        private void Ejecutar_filtro(string campo_busqueda, string condi_busqueda, DateTime FecIni, DateTime FecFin)
        {
            ...
            ENResultOperation R = ClsMantenimiento_PreventivoBC.Listar_Filtro(campo_busqueda, condi_busqueda, FecIni, FecFin);
