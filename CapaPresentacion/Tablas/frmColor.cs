...
namespace CapaPresentacion.Tablas
{
    public partial class frmColor : Form
    {
        ...
        void FormatoDgv()
        {
            ...
            dgv.Columns[0].DataPropertyName = "COLOR_IDE";

            dgv.Columns[1].Width = 340;
            dgv.Columns[1].HeaderText = "Descripción ";
            dgv.Columns[1].DataPropertyName = "COLOR_NOMBRE";

            dgv.Columns[2].Width = 85;
            dgv.Columns[2].HeaderText = "Estado";
            dgv.Columns[2].DataPropertyName = "COLOR_ESTADO";

            // Columnas que no Deben Verse

            dgv.Columns[3].DataPropertyName = "COLOR_FECHAINAC";
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsColorBC.ListarTodos(filtro);
            if (R.Proceder)
            {
        ...
        {
            ClsColorBE TipoBE = new ClsColorBE();
            TipoBE.Color_ide = Convert.ToInt32(txtIde.Text);
            TipoBE.Color_nombre = txtNombre.Text;
            TipoBE.Color_estado = cboEstado.Text;
            TipoBE.Color_fechainac = Convert.ToDateTime("01-01-1900");
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsColorBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Color : " + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsColorBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Color : " + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsColorBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Color : " + R.Sms);
                        break;
                    }
            }
