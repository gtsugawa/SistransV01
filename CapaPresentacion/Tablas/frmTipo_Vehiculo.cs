...
namespace CapaPresentacion.Tablas
{
    public partial class frmTipo_Vehiculo : Form
    {
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsTipo_VehiculoBC.Listar(filtro);
            if (R.Proceder)
            {
        ...
        {
            ClsTipo_VehiculoBE TipoBE = new ClsTipo_VehiculoBE();
            TipoBE.Tipo_vehi_ide = Convert.ToInt32(txtIde.Text);
            TipoBE.Tipo_vehi_nombre = txtNombre.Text;
            TipoBE.Tipo_vehi_estado = cboEstado.Text;
            TipoBE.Tipo_vehi_fechainac = Convert.ToDateTime("01-01-1900");
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsTipo_VehiculoBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Tipo Vehiculo : " + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsTipo_VehiculoBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Tipo Vehiculo : " + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsTipo_VehiculoBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Tipo Vehiculo : " + R.Sms);
                        break;
                    }
            }
            Estado_Botones(true);
