...
{
    public partial class frmSerie_Orden_Recojo : Form
    {
        string Operacion = null;  // Operaciones : N = Nuevo / M = Modificar E = Eliminar
        string Serie_Anterior = null;
         public frmSerie_Orden_Recojo()
        {
            InitializeComponent();
        }

        private void frmSerie_Orden_Recojo_Load(object sender, EventArgs e)
        {
            FormatoDgv();
            Mostrar_dgv("");
            llenar_CboEstado();
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsSerie_Orden_RecojoBC.Listar(filtro);
            if (R.Proceder)
            {
        ...
        {
            ClsSerie_Orden_RecojoBE TipoBE = new ClsSerie_Orden_RecojoBE();
            ...
            TipoBE.Serie_numero_anterior = Serie_Anterior;
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsSerie_Orden_RecojoBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Serie de Orden de Recojo : " + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsSerie_Orden_RecojoBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Insertar Serie de Orden de Recojo : " + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsSerie_Orden_RecojoBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Serie de Orden de Recojo : " + R.Sms);
                        break;
                    }
            }
