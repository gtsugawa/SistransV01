...
{
    public partial class frmMarca_Vehiculo : Form
    {
        string Operacion = null;  // Operaciones : N = Nuevo / M = Modificar E = Eliminar
        string Mens_Error = "";
        Boolean Flg_Retorno = true;
        public frmMarca_Vehiculo()
        {
            InitializeComponent();
        }

        private void frmMarca_Vehiculo_Load(object sender, EventArgs e)
        {
            FormatoDgv();
            Mostrar_dgv("");
            llenar_CboEstado();
        ...
        void FormatoDgv()
        {
            ...
            dgv.Columns[0].DataPropertyName = "MARCA_VEHI_IDE";

            dgv.Columns[1].Width = 340;
            dgv.Columns[1].HeaderText = "Descripción ";
            dgv.Columns[1].DataPropertyName = "MARCA_VEHI_NOMBRE";

            dgv.Columns[2].Width = 85;
            dgv.Columns[2].HeaderText = "Estado";
            dgv.Columns[2].DataPropertyName = "MARCA_VEHI_ESTADO";

            // Columnas que no Deben Verse

            dgv.Columns[3].DataPropertyName = "MARCA_VEHI_FECHAINAC";
        ...
        private void Mostrar_dgv(string filtro)
        {
            ...
            ENResultOperation R = ClsMarca_VehiculoBC.ListarTodos(filtro);
            if (R.Proceder)
            {
        ...
        {
            ClsMarca_VehiculoBE TipoBE = new ClsMarca_VehiculoBE();
            TipoBE.Marca_vehi_ide = Convert.ToInt32(txtIde.Text);
            TipoBE.Marca_vehi_nombre = txtNombre.Text;
            TipoBE.Marca_vehi_estado = cboEstado.Text;
            TipoBE.Marca_vehi_fechainac = Convert.ToDateTime("01-01-1900");
            ...
            switch (Operacion)
            {
                case "N":
                    {
                        ENResultOperation R = ClsMarca_VehiculoBC.Crear(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Insertar Marca Vehiculo : " + R.Sms);
                        break;
                    }
                case "M":
                    {
                        ENResultOperation R = ClsMarca_VehiculoBC.Actualizar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Modificar Marca Vehiculo : " + R.Sms);
                        break;
                    }
                case "E":
                    {
                        ENResultOperation R = ClsMarca_VehiculoBC.Eliminar(TipoBE);
                        if (!R.Proceder) MessageBox.Show("Error al Eliminar Marca Vehiculo : " + R.Sms);
                        break;
                    }
            }
